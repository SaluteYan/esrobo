...
bool FTSerialDriver::OpenPort(const std::string &_port, int _baudrate, DataBits _dbits, Parity _parity, StopBits _stopbits)
{
	char parity;
...
int FTSerialDriver::SetPort(int iBaudRate, int iDataSize, char cParity, int iStopBit)
{
    int iResult = 0;
...
int FTSerialDriver::ReadPort(void *buf, int iByte)
{
    int iLen = 0;
    if(!iByte)
    {
...
bool FTSerialDriver::WritePort(const void *buf, int iByte)
{
    int iLen = 0;
    if(!iByte)
    {
...
int FTSerialDriver::GetComListFromReg(std::vector<std::string> &_PortList)
{
