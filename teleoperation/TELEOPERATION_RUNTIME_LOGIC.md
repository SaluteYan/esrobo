...
## 分布式运行新增路径（2026-09-22）

新增 [`robot_link`](../robot_link/README.md)，与下文原有一体式 `TeleopNode` 入口并存。
通信字段和状态机的唯一详细说明为 [`PROTOCOL.md`](../robot_link/PROTOCOL.md)，验证记录见
[`DEVELOPMENT.md`](../robot_link/DEVELOPMENT.md)。收到关节目标后的逐步执行、轨迹计算、CAN/手部桥下发和反馈语义，
见 [`ROBOT_EXECUTION.md`](../robot_link/ROBOT_EXECUTION.md)。

1. 笔记本采集 PICO/SenseGlove、执行标定/重定向/IK，发送选定侧七轴 URDF 弧度和可选 L10
   十轴物理整数目标。SDK 和接入示例已提供，完整笔记本应用尚未迁移。
2. 机器人 `Gateway` 的 UDP 线程约 50 Hz 返回状态。HMAC、会话、递增序号、配置合同、目标范围
   和机器人签发的 200 ms 有效令牌均通过后，只覆盖最新目标。迟到目标不获得新的完整有效期。
3. 单独串行执行线程复用 `NeroSingleArmDriver` 的反馈闭环限制及控制器/力矩检查；每周期发送前
   重新读取反馈。模型仅做 FK/末端速度约束、启动检查和安全回零，不运行人体 IK。
4. `e` 仅机器人终端明确触发，首目标需匹配实测姿态。使能等待和力矩基线可被取消；网络丢失、
   反馈过期、执行异常均闭锁，不因重连自动恢复。实际停止和失能只能由新鲜反馈证明。
5. 手指物理目标经新增 `command_physical` 接口执行，保留原速度、步长、活动关节掩码和反馈门禁；
   停止时冻结手部目标，不自动张手。`z` 使用原安全回零规划及几何标定要求，返回成功且失能才回 IDLE。
6. 图像由另一个进程采集/转发最新彩色 JPEG，经 SSH 转发本机 HTTP；显示与分析在笔记本。
   图像不占关节数据包，不包含深度或内参。默认状态记录保存到 `robot_link/log`，不进入 Git。

支持单侧及 `--side both` 双臂＋双手网关；双侧共用一会话、一手部反馈接收器，整包校验四目标，
依次发送两臂后整包发送双手，任一故障闭锁全部。两臂使能期间继续监测先使能侧。
双侧 `z` 暂不支持：现有回零几何未覆盖两臂互撞，拒绝自动拼接路径。单侧回零不变。
不能与旧控制进程共享 CAN。保持原有运动参数；
该拆分减少机器人主机上层计算与显示负载，尚不构成主机崩溃原因已消除或实机验收通过的证据。

## SenseGlove ROS 中间件隔离

SenseGlove 联合启动器默认导出 `RMW_IMPLEMENTATION=rmw_cyclonedds_cpp`，并在配置任何机器人硬件前
检查对应 ROS 包是否存在。原因是 2026-09-21 的左手联合启动中，`robot_state_publisher` 已在
`libfastrtps.so.2.6.11` 内发生 general protection fault，`ros2_control_node` 随后也收到 `SIGSEGV`。
所有由启动器创建的 ROS 节点继承同一个 Cyclone DDS 环境，避免同一进程图混用 RMW。需要对照诊断时
可显式设置 `SENSEGLOVE_RMW_IMPLEMENTATION`，但实机入口不再默认走已经崩溃过的 Fast DDS 路径。

SenseCom 仍使用独立 SGCore IPC，不经过 ROS DDS。启动器以 niceness 10 运行其 Unity 进程，降低其
持续占用单核 CPU 时对 PICO 数据桥和网页预览的调度影响；手套数据有效性仍以 `connected=true` 和
`rx>0` 为准。

> 最近核对日期：2026-09-21
> 当前重点入口：PICO 单臂＋同侧 SenseGlove＋同侧 LinkerHand 联动遥操作
...
./scripts/run_pico_right_arm_hand_teleop.sh --right-serial 00892 --recalibrate
```

该入口最终进入 `run_senseglove_hand_teleop.sh --pico-right-arm-hand`，只连接右侧设备：

| 输入或设备 | 当前职责 |
|---|---|
| PICO 右肩、右肘、右腕位置 | 生成右臂 J1-J4 目标 |
| 右 SenseGlove IMU | 生成右臂 J5-J7 相对方向目标 |
| 右 SenseGlove 手指数据 | 生成右手 10 个主动关节目标 |
| 右 NERO | `can_piper2`，执行七轴位置命令 |
| 右 LinkerHand L20Lite/L10 接口 | `can_hand1`，接收 10 个 0–255 物理执行器目标 |
| 左臂、左手套、左灵巧手 | 不初始化、不发送命令 |

联合入口强制重新采集本次手套上电后的六步手指/IMU 标定，禁止 `--enable` 自动使能。SenseGlove 标定只建立手套输入到目标的映射；它不等于 LinkerHand 反馈值到 URDF 角度的碰撞几何标定。

### 1.2 联合入口的启动顺序

1. 只读启动 XRoboToolkit，确认 PICO 右肩、右肘和右腕位置连续更新。
2. 启动或复用 SenseCom，按序列号确认右手套连接且接收计数增长。
3. 配置 `can_hand1`，启动右 LinkerHand 驱动；驱动设置 `move_on_start=false`，此时不应产生启动动作。
4. 启动 `hand_ros_bridge.py --side right`，核验 `/cb_right_hand_state` 为 10 个有效的 0–255 数值。
5. 执行右手套六步标定并启动连续 UDP 桥。单手模式在 20 维报文中镜像填充未使用侧，但物理驱动和命令桥只启用右侧，左手不会收到命令。
6. 配置右臂 `can_piper2`，进入 PICO 参考采集；此时右臂和右手跟随都未使能。
7. 核验右臂自然下垂零位和右手自然张开反馈。只有操作者按 `e` 后才执行联合使能门禁。

后台进程退出时按进程组清理。`hand_ros_bridge` 在 ROS 信号处理已经关闭上下文时不再重复调用 `rclpy.shutdown()`，避免把正常退出记录成 `rcl_shutdown already called` 异常。

### 1.3 按 `e` 的联合门禁和运行周期

联合使能顺序固定如下：

1. 检查最新 20 维手指目标存在；右侧目标过期时拒绝。
2. 检查右 LinkerHand 十轴反馈完整、范围为 0–255，且接收时间不超过 `0.5 s`。
3. 检查右手套 IMU 新鲜，PICO 目标已经通过稳定窗口。
4. 右臂按单臂流程完成实测姿态、控制器状态、碰撞等级、力矩基线和轨迹初始化检查。
5. 右臂成功进入跟随后，才以当前右手实测反馈作为第一条手部保持目标，使能右手跟随。
6. 如果第 5 步期间右手反馈消失，立即冻结手指目标并锁住右臂跟随，不把只有机械臂或只有手的半完成状态报告为联合使能成功。

运行循环理论频率为 50 Hz。机械臂每周期计算 J1-J7；右手最多以 `hand.publish_hz=50 Hz` 下发。每个物理手指执行器单帧变化同时受 `max_step=18` 和 `max_velocity=400 unit/s` 约束；在 50 Hz 下速度约束对应每帧最多 8 个单位，因此实际更严格的是 8/255。当前允许主动跟随的物理索引为 `[0,1,2,3,4,5,9]`，未验证的 6/7/8 轴保持最新实测位置。

右手命令映射使用独立的自然张开和遥操作闭合端点。反馈门禁使用 `right_open_feedback`，不会拿命令端点反推实测角度。手套数据过期会先关闭右手跟随，再锁住右臂；LinkerHand 反馈过期会使手部驱动自行撤销使能，并触发同样的联合安全停止。恢复不会自动发生，必须排障后走 `z` 或重新启动。
...
```bash
cd /home/esrobo/Projects/esrobo/teleoperation
./scripts/run_pico_left_arm_hand_teleop.sh --left-serial 00885 --recalibrate
```

它与右侧使用同一套联合状态机，但所有物理资源都限定在左侧：PICO 左肩、左肘和左腕位置驱动左臂 J1-J4，左 SenseGlove IMU 驱动 J5-J7，左手指数据驱动左 LinkerHand。机械臂使用 `can_piper1`，灵巧手使用 `can_hand2`，ROS 反馈/命令分别为 `/cb_left_hand_state` 和 `/cb_left_hand_control_cmd`；右臂、右手套和右灵巧手不初始化。

左侧有两项独立调试条件，不能套用右手数据：

1. `left_open_feedback` 沿用已经用于左灵巧手单独遥操作启动和退出回张的
...
需要重新核对或替换兼容零位时，使用 `scripts/capture_hand_open_feedback.py --side left` 独立采集。
该工具只订阅 `/cb_left_hand_state`，不创建控制命令发布器；它以多帧中值生成候选值，并在任一通道
采集跨度过大时拒绝输出可用标定。工具不会自动写配置，避免把错误姿态或短时噪声永久写入启动零位。
...
| `s` | 正常状态下启动安全回零；联合模式先冻结手指，当前侧机械臂回零失能确认后才张开同侧灵巧手；故障闭锁时只报告停止已经生效；回程中按 `s` 会取消回程 |
| `z` | 人工排障后的显式检查回零，不恢复跟随 |
| `d` | 请求七轴失能，不发送回零位置 |
| `x` | 发送电子急停 |
| `h` | 联合模式只切换当前侧灵巧手跟随，不改变机械臂使能状态 |
| `q` | 正常或故障闭锁状态下请求受检回零；联合模式在当前侧机械臂零位和失能确认后才尝试张开同侧灵巧手并退出。回程中按 `q` 只登记成功后退出，不取消回程 |
...
- `linker_hand_driver.log`：当前侧手部 SDK、CAN、固件版本和驱动错误；
- `hand_ros_bridge.log`：当前侧 ROS2 命令/反馈与 UDP 桥；
- `senseglove_driver.log`：当前侧手套 ROS2 驱动和控制器启动；
- `senseglove_bridge.log`：手指目标、IMU 修复、目标发送频率和手部骨架状态；
- `pico_left_arm_bridge.log` 或 `pico_right_arm_bridge.log`：当前侧 PICO 手臂 UDP 数据桥。
...
- 当前文档覆盖 PICO 左右单臂及单臂＋同侧手联合入口；双臂同时跟随和独立腕部 IMU 模式不能直接套用全部结论。
