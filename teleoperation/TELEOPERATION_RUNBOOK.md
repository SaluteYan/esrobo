...
零位核对；活动手指的反馈到 URDF 几何映射是另一项门禁。当前 L20Lite/L10 十轴映射已写入配置；若机器人仍加载旧配置，联合入口会
...
联动模式中，PICO 控制机械臂、手腕和五指。活动手指会加入躯干碰撞包络；网页必须显示所选侧几何映射 `10/10` 才允许提交联合使能。缺少或无效映射时程序按完整手指运动范围检查并拒绝启动。不要通过缩小碰撞余量或把未知手指角度假定为零来绕过拒绝。
