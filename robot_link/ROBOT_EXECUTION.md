...
| `RETURNING` | 可以，但不用于回零 | 单侧按本地已检查路径回零；目标流不驱动此过程 |

`e` 需要本机终端明确输入并回车。真实后端先确认七轴新鲜反馈、七轴失能状态、控制器状态正常，
且首目标与实测臂位置逐轴相差不超过 1.5°。带手模式还要求独立开位反馈标定、实测自然张开位和首手目标匹配。
上电静默、无法读取反馈时不会盲目使能。

机械臂驱动随后检查启动姿态几何、配置并核验控制器碰撞等级、预载实测姿态、使能和确认状态，
以保持姿态采集力矩基线，再初始化命令轨迹。当前基线先等待 1 秒，再要求 15 次有效采样，采样超时为 4 秒。
基线是当前保持姿态的测量参考，不要求把机械臂自动移动到零位。网关再使用原配置速度；首个正常跟随周期仍保持初始化位置。
...
故障后新目标或重连都不能解除闭锁。单侧按原排障/`z` 恢复；双侧需现场排障后重启网关，不能拼接单臂回零绕过对侧障碍。
