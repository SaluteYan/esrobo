...
**支持单侧机械臂、可选同侧 L10 灵巧手，以及双臂＋双手。** 单侧使用 `--side left/right`，四部件联合使用 `--side both`（自动包含双手）。所有模式默认模拟，真实硬件必须显式 `--hardware`。双侧必须使用一个网关，禁止启动两个单侧网关；尚无单手独立网关。腰部、头部保持原模型固定前提。
笔记本完整 PICO/SenseGlove/IK/网页入口尚未迁移，本版 SDK 与示例是后续接入的边界；旧脚本仍是原来的本机一体运行方式。
...
15051/反馈端口只供机器人本机桥使用，笔记本统一连接 16000。手指目标仍经过本机活动轴掩码、步长和速度限制。反馈零位与手指几何标定是不同配置：缺少可靠几何时启动/回零可能拒绝，网络模式不跳过这些检查。
...
首次 `e` 前需要：新鲜七轴反馈、确认七轴失能、目标流与实测姿态一致（1.5° 内）。臂手模式还要求手已处于经过反馈核验的自然张开位、首帧手目标与反馈一致。控制器上电不发反馈时明确拒绝；本版不自动“唤醒使能”，先按旧手册在现场解决反馈问题。
...
- 任一侧故障、断流或 `s/x/q` 都请求两臂停止，冻结双手新目标。第一臂停止发送失败仍尝试第二臂，`d` 也会尝试失能两臂；是否成功以反馈为准。
...
`state["feedback"]["sides"]["left"/"right"]`。笔记本完整采集、IK 和网页接入仍是后续工作。
...
`stop_send_returned=true` 只表示调用返回，不是机械停止/失能证明。看实测反馈、七轴 `enable_states`、控制器状态；反馈过期时状态为未知。手部停止继续发新目标，保留原目标，不自动张开。单侧故障后须现场排障，独立 `z` 成功回零/失能后才重新 `e`；双侧按前文排障并重启。重连不自动恢复。
