...
| 通道 | 地址/频率 | 数据 |
| --- | --- | --- |
| 关节控制 | 机器人 UDP 16000；笔记本临时 UDP 端口 | hello、target、stop |
...
stop 与 target 使用同一序号空间和认证。没有网络 enable/disable/return 指令，避免计算程序重连后自动改变执行状态。远程 stop 丢失或无法发送时，跟随有效期耗尽仍会触发停止；停止请求不等于物理停止证明。
